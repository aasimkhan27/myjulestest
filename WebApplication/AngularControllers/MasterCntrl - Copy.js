app.controller('MasterController', function ($scope, $http, $cookies, $filter, CommService, $window, $localStorage, PrcCommMethods, $location) {
    ...
    //$scope.EMAIL_PATTERN = /^[_a-z0-9]+(\.[_a-z0-9]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,15})$/;
    ...
    $scope.CheckUserLogin = function () {
        if ($cookies.get("CREATED_BY") == "999999999") {

        }
        else if ($cookies.get("USERID") == undefined || $cookies.get("USERID") == '' || $cookies.get("USERID") == null || $cookies.get("USERID") == "null" || parseInt($cookies.get("USERID")) == 0) {
    ...
    if (window.location.href.toLowerCase().indexOf("plain/index") != -1) {
        ...
        if (window.location.href.toLowerCase().indexOf("plain/index#!/clock_in_out") != -1) {
            ...
            if ($filter('lowercase')($location.absUrl()).indexOf("basic_details") != -1 || $filter('lowercase')($location.absUrl()).indexOf("enter_sites") != -1) {
    ...
    $scope.SelectEntity = function (FQ, PATH_FLAG, USER_ID) {
        ...
        if (PATH_FLAG == 'BASIC_HR') {
            $cookies.put('USERID', USER_ID, { 'path': '/' });
            //$cookies.put('USER_ID_HR', getUrlParameter('ID', $location.absUrl()), { 'path': '/' }); 
            $location.path('Organisation').search('ID', USER_ID);
            $scope.HIDE_HEADER_DETAIL = true;
        }
    ...
    $scope.MASTER_GET_CUSTOMER_SETTINGS = function (_param_tableids, calling_from_other_controller) {
    ...
    $scope.GET_ENTITY_MODULE_ROLES = function (Module) {
        ...
        var httpRequest = $http({
            ...
            if ($cookies.get("USERID") == 31 || data.data != null && data.data.Table.length > 0) {
                ...
                if (Module.MODULE_ID == 20) {
                    ...
                    $scope.MASTER_GET_CUSTOMER_SETTINGS('46,47,52,61,87,89,54'); // same function calling general controller, function (HRM_INS_UPD_HOLIDAY_CALENDAR_BRANCH_MAPPING). you need to update both places
                }
                else {
                    window.location.href = Module.REDIRECT_PATH;
                }
                ...
                // }
                //else {
                //   window.open(Module.SUBSCRIBE_PATH);
                //}
            }
            else {
                if (!Module.ROLE_REQUIRED) {
                    ...
                    alert("You donot have access to " + Module.MODULE_NAME + ".");
                }

            }
        });
    }
    $scope.GET_ENTITY_MODULES = function (ENTITY_ID, ASIDE_FLAG) {
    ...
    $scope.ModuleRedirection = function (Module) {
        $scope.SELECTED_MODULE = Module;
        $localStorage.SELECTED_MODULE = JSON.stringify(Module);
        $localStorage.SUB_MODULE_LIST = undefined;
        if (Module.FLAG) {
    ...
    $scope.Redirect = function (path, ModuleName, RoleIDs) {
        if (CheckAccess(RoleIDs)) {
            ...
            alert("You donot have access to " + ModuleName + ".");
        }
    }
    ...
    $scope.DELETE_UPLOAD_ALL = function (Array, item, index, FLAG) {
        if (confirm('Are you sure you want to delete the file?')) {
            ...
            PrcCommMethods.CASHUP_API(CashupModelObj, 'DELETE_UPLOAD').then(function (data) {
                $scope.ShowAlert('Success', 'Delete Successfully', 5000);
                if (FLAG == 1) {
