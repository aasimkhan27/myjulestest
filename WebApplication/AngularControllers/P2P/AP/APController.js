app.controller('APController', function ($scope, $http, CommService, $cookies, $filter, PrcCommMethods, $location, $localStorage, pdfDelegate, $interval) {
    ...
    if (window.location.href.toLowerCase().indexOf("p2p/p2pindex#!/p2pap") != -1) {
        $scope.APViewSearch.APPROVAL_TYPE_ID = 4;
    }
    ...
    $scope.APViewSearch.NOTE_FOR_APPROVERS_LIMIT_TO = 500;
    ...
    $scope.GET_INVOICES_ACTION = function (FLAG, SORT_COLUMN_NO, SORT_ORDER_NO_ASCDESC, SYNC_FLAG) {
        ...
        ModelObj.BRANCH_IDS = $scope.APViewSearch.BRANCH_IDS
        ModelObj.STATUS_IDS = "";//$scope.APViewSearch.STATUS_IDS == null ? '37, 38, 39' : $scope.APViewSearch.STATUS_IDS;
        ModelObj.SEARCH_PARAMETER = $scope.APViewSearch.SEARCH_NAME;
        ModelObj.PAGE_NO = $scope.APViewSearch.PAGE_NO;
        ModelObj.PAGE_SIZE = $scope.APViewSearch.PAGE_SIZE;
        ModelObj.INVOICE_TYPE_ID = 1;// --1 AP INVOICE, 2 AP CREDIT NOTE
        ...
        PrcCommMethods.P2P_API(ModelObj, 'GET_INVOICES', 'PO').then(function (data) {
            if (data.data.Table.length > 0) {
                ...
                if (data.data.Table.length < $scope.APViewSearch.PAGE_SIZE) {
                    $scope.GetActionData = false;
                }
                else {
                    $scope.APViewSearch.PAGE_NO = parseInt($scope.APViewSearch.PAGE_NO) + 1;
    ...
    $scope.GET_INVOICES_PROCESS = function (FLAG, SORT_COLUMN_NO, SORT_ORDER_NO_ASCDESC) {
        if (FLAG == 1) {
            $scope.APViewSearch.PAGE_NO_PROCESS = 1;
        ...
        ModelObj.BRANCH_IDS = $scope.APViewSearch.BRANCH_IDS
        ModelObj.STATUS_IDS = "";// $scope.APViewSearch.STATUS_IDS == null ? '37, 38, 39' : $scope.APViewSearch.STATUS_IDS;
        ModelObj.PAGE_NO = $scope.APViewSearch.PAGE_NO_PROCESS;
        ModelObj.PAGE_SIZE = $scope.APViewSearch.PAGE_SIZE;
        ModelObj.SEARCH_PARAMETER = $scope.APViewSearch.PROCESS_SEARCH_NAME;
        ModelObj.INVOICE_TYPE_ID = 1;// --1 AP INVOICE, 2 AP CREDIT NOTE
        PrcCommMethods.P2P_API(ModelObj, 'GET_INVOICES', 'PO').then(function (data) {
            if (data.data.Table.length > 0) {
                ...
                if (data.data.Table.length < $scope.APViewSearch.PAGE_SIZE) {
                    ...
                    $scope.APViewSearch.PAGE_NO_PROCESS = parseInt($scope.APViewSearch.PAGE_NO_PROCESS) + 1;
    ...
    $scope.GET_INVOICES_ADMIN = function (FLAG, SYNC_FLAG) {
        if (FLAG == 1) {
            $scope.APViewSearch.PAGE_NO_ALL = 1;
        ...
        ModelObj.BRANCH_IDS = $scope.APViewSearch.BRANCH_IDS
        ModelObj.STATUS_IDS = "";
        ModelObj.PAGE_NO = $scope.APViewSearch.PAGE_NO_ALL;
        ModelObj.PAGE_SIZE = $scope.APViewSearch.PAGE_SIZE;
        ModelObj.SEARCH_PARAMETER = $scope.APViewSearch.SEARCH_NAME_ALL;
        ModelObj.INVOICE_TYPE_ID = 1;// --1 AP INVOICE, 2 AP CREDIT NOTE
        PrcCommMethods.P2P_API(ModelObj, 'GET_INVOICES', 'PO').then(function (data) {
            if (data.data.Table.length > 0) {
                ...
                if (data.data.Table.length < $scope.APViewSearch.PAGE_SIZE) {
                    ...
                    $scope.APViewSearch.PAGE_NO_ALL = parseInt($scope.APViewSearch.PAGE_NO_ALL) + 1;
    ...
    $scope.GET_PENDING_OUTBOUND_PROCESSING_INVOICES = function (FLAG) {
        if (FLAG == 1) {
            $scope.APViewSearch.OUT_PAGE_NO = 1;
            $scope.PENDING_OUTBOUND_PROCESSING_INVOICES_LIST = [];
        }
        ModelObj = new Object();
        ModelObj.SEARCH_PARAMETER = $scope.APViewSearch.SEARCH_NAME_OUT;
        ModelObj.STATUS_IDS = $scope.APViewSearch.STATUS_IDS;
        ModelObj.ENTITY_ID = parseInt($cookies.get("ENTITY_ID"));
        ModelObj.BRANCH_IDS = $scope.APViewSearch.BRANCH_IDS;
        ModelObj.USER_ID = parseInt($cookies.get("USERID"));
        ModelObj.PAGE_NO = $scope.APViewSearch.OUT_PAGE_NO;
        ModelObj.PAGE_SIZE = $scope.APViewSearch.PAGE_SIZE;
        ModelObj.INVOICE_TYPE_ID = 1;// --1 AP INVOICE, 2 AP CREDIT NOTE
        PrcCommMethods.P2P_API(ModelObj, 'GET_PENDING_OUTBOUND_PROCESSING_INVOICES', 'PO').then(function (data) {
            if (data.data.Table.length > 0) {
                ...
                if (data.data.Table.length < $scope.APViewSearch.PAGE_SIZE) {
                    ...
                    $scope.APViewSearch.OUT_PAGE_NO = parseInt($scope.APViewSearch.OUT_PAGE_NO) + 1;
    ...
    $scope.GET_INVOICE_BY_ID = function (REQUEST_LINE, FLAG) {
        ...
        PrcCommMethods.P2P_API(ModelObj, 'GET_INVOICE_BY_ID', 'PO').then(function (data) {
            if (data.data.Table.length > 0) {
                ...
                if (FLAG == 1) {
                    $scope.APViewSearch.REQUEST_DETAILS = data.data.Table[0];
                    $scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE = ($scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE == "" || $scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE == null || $scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE == undefined) ? $scope.APViewSearch.REQUEST_DETAILS.INVOICE_DATE : data.data.Table3.length > 0 ? data.data.Table3[0].INVOICE_DATE : $scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE;
                    $scope.APViewSearch.REQUEST_DETAILS.IS_MULTI_FILE_UPLOAD_ALLOW = true;
                    $scope.APViewSearch.REQUEST_DETAILS.INVOICE_HEADER_ID = REQUEST_LINE.INVOICE_HEADER_ID;
                    $scope.APViewSearch.REQUEST_DETAILS.APPROVAL_HEADER_ID = REQUEST_LINE.APPROVAL_HEADER_ID;
                    REQUEST_LINE.ACTION_ITEM_LIST = data.data.Table1;
                    $scope.ACTION_ITEM_LIST = REQUEST_LINE.ACTION_ITEM_LIST;
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.REQUEST_DETAILS, 33, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.VIEW_INVOICE_DETAIL($scope.APViewSearch.REQUEST_DETAILS, 2, 1);
                    //$scope.INVOICE_SPLIT_LIST = data.data.Table2
                    $scope.$parent.overlay_loadingNew = 'none';
                }
                else if (FLAG == 2) {

                    $scope.APViewSearch.PROCESS_REQUEST_DETAILS = data.data.Table[0];
                    $scope.APViewSearch.PROCESS_REQUEST_DETAILS.BUDGET_DATE = ($scope.APViewSearch.PROCESS_REQUEST_DETAILS.BUDGET_DATE == "" || $scope.APViewSearch.PROCESS_REQUEST_DETAILS.BUDGET_DATE == null || $scope.APViewSearch.PROCESS_REQUEST_DETAILS.BUDGET_DATE == undefined) ? $scope.APViewSearch.PROCESS_REQUEST_DETAILS.INVOICE_DATE : data.data.Table3.length > 0 ? data.data.Table3[0].INVOICE_DATE : $scope.APViewSearch.PROCESS_REQUEST_DETAILS.BUDGET_DATE;

                    $scope.APViewSearch.PROCESS_REQUEST_DETAILS.INVOICE_HEADER_STATUS_ID = REQUEST_LINE.INVOICE_HEADER_STATUS_ID;
                    $scope.APViewSearch.PROCESS_REQUEST_DETAILS.SHORT_NAME = $scope.$parent.TextReturn($scope.APViewSearch.PROCESS_REQUEST_DETAILS.REQUESTOR_NAME);
                    $scope.APViewSearch.PROCESS_REQUEST_DETAILS.PO_ATTACHMENT_IN_INVOICE = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    $scope.APViewSearch.PROCESS_REQUEST_DETAILS.GRN_ATTACHMENT = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    $scope.APViewSearch.PROCESS_REQUEST_DETAILS.OTHER_ATTACHMENT = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.PROCESS_REQUEST_DETAILS, 33, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.PROCESS_REQUEST_DETAILS.PO_ATTACHMENT_IN_INVOICE, 37, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.PROCESS_REQUEST_DETAILS.GRN_ATTACHMENT, 38, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.PROCESS_REQUEST_DETAILS.OTHER_ATTACHMENT, 39, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.VIEW_INVOICE_DETAIL($scope.APViewSearch.PROCESS_REQUEST_DETAILS, 2, 1);
                    $scope.$parent.overlay_loadingNew = 'none';
                    ...
                    $scope.APViewSearch.ALL_REQUEST_DETAILS = data.data.Table[0];
                    $scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE = ($scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE == "" || $scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE == null || $scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE == undefined) ? $scope.APViewSearch.ALL_REQUEST_DETAILS.INVOICE_DATE : data.data.Table3.length > 0 ? data.data.Table3[0].INVOICE_DATE : $scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE;

                    $scope.APViewSearch.ALL_REQUEST_DETAILS.IS_MULTI_FILE_UPLOAD_ALLOW = true;
                    $scope.APViewSearch.ALL_REQUEST_DETAILS.INVOICE_HEADER_STATUS_ID = REQUEST_LINE.INVOICE_HEADER_STATUS_ID;
                    $scope.APViewSearch.ALL_REQUEST_DETAILS.SHORT_NAME = $scope.$parent.TextReturn($scope.APViewSearch.ALL_REQUEST_DETAILS.REQUESTOR_NAME);
                    $scope.APViewSearch.ALL_REQUEST_DETAILS.PO_ATTACHMENT_IN_INVOICE = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    $scope.APViewSearch.ALL_REQUEST_DETAILS.GRN_ATTACHMENT = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    $scope.APViewSearch.ALL_REQUEST_DETAILS.OTHER_ATTACHMENT = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    REQUEST_LINE.ALL_ITEM_LIST = data.data.Table1;
                    $scope.ALL_ITEM_LIST = REQUEST_LINE.ALL_ITEM_LIST;
                    // $scope.GET_APPROVAL_CUSTOM_FIELD_OPTIONS_MASTER(REQUEST_LINE, 2);
                    $scope.VIEW_INVOICE_DETAIL($scope.APViewSearch.ALL_REQUEST_DETAILS, 2, 1);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.ALL_REQUEST_DETAILS, 33, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.ALL_REQUEST_DETAILS.PO_ATTACHMENT_IN_INVOICE, 37, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.ALL_REQUEST_DETAILS.GRN_ATTACHMENT, 38, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.ALL_REQUEST_DETAILS.OTHER_ATTACHMENT, 39, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.overlay_loadingNew = 'none';
                    ...
                    $scope.APViewSearch.OUT_REQUEST_DETAILS = data.data.Table[0];
                    $scope.APViewSearch.OUT_REQUEST_DETAILS.IS_MULTI_FILE_UPLOAD_ALLOW = true;
                    $scope.APViewSearch.OUT_REQUEST_DETAILS.OVERALL_STATUS_ID = REQUEST_LINE.OVERALL_STATUS_ID;
                    $scope.APViewSearch.OUT_REQUEST_DETAILS.SHORT_NAME = $scope.$parent.TextReturn($scope.APViewSearch.OUT_REQUEST_DETAILS.REQUESTOR_NAME);
                    $scope.APViewSearch.OUT_REQUEST_DETAILS.PO_ATTACHMENT_IN_INVOICE = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    $scope.APViewSearch.OUT_REQUEST_DETAILS.GRN_ATTACHMENT = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    $scope.APViewSearch.OUT_REQUEST_DETAILS.OTHER_ATTACHMENT = { IS_MULTI_FILE_UPLOAD_ALLOW: true, };
                    REQUEST_LINE.OUT_ITEM_LIST = data.data.Table1;
                    $scope.OUT_ITEM_LIST = REQUEST_LINE.OUT_ITEM_LIST;
                    $scope.VIEW_INVOICE_DETAIL($scope.APViewSearch.OUT_REQUEST_DETAILS, 2, 1);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.OUT_REQUEST_DETAILS, 33, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.OUT_REQUEST_DETAILS.PO_ATTACHMENT_IN_INVOICE, 37, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.OUT_REQUEST_DETAILS.GRN_ATTACHMENT, 38, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.GET_UPLOADS($scope.APViewSearch.OUT_REQUEST_DETAILS.OTHER_ATTACHMENT, 39, REQUEST_LINE.INVOICE_HEADER_ID);
                    $scope.$parent.overlay_loadingNew = 'none';
                    $scope.INVOICE_PROCESS_SPLIT_LIST = data.data.Table3;
    ...
    $scope.GET_PO_SPLIT_DATES_BY_INVOICE_ID = function (REQUEST_LINE) {
        ...
        PrcCommMethods.P2P_API(ModelObj, 'GET_PO_SPLIT_DATES_BY_INVOICE_ID', 'PO').then(function (data) {
            if (data.data.Table.length > 0) {
                var LINE = $scope.APViewSearch.REQUEST_DETAILS;
    ...
    $scope.GET_PO_FOR_INVOICE_PO_MAPPING = function (FLAG, LOAD_FLAG) {
        var PaymentModelObj = new Object();
        PaymentModelObj.CONTACT_ID = $scope.TAB_FLAG == 1 ? $scope.APViewSearch.REQUEST_DETAILS.CONTACT_ID : $scope.APViewSearch.ALL_REQUEST_DETAILS.CONTACT_ID;    //@  INT,
        PaymentModelObj.CURRENCY_ID = $scope.TAB_FLAG == 1 ? $scope.APViewSearch.REQUEST_DETAILS.CURRENCY_ID : $scope.APViewSearch.ALL_REQUEST_DETAILS.CURRENCY_ID;  //@ INT,
        PaymentModelObj.AMOUNT_FROM = $scope.APViewSearch.AMOUNT_FROM;//@ NUMERIC(18, 5), 
        PaymentModelObj.AMOUNT_TO = $scope.APViewSearch.AMOUNT_TO; //@ NUMERIC(18, 5), 
        PaymentModelObj.DATE_FROM = $scope.APViewSearch.DATE_FROM;  //@ DATE,
        PaymentModelObj.DATE_TO = $scope.APViewSearch.DATE_TO;  //@ DATE,
        PaymentModelObj.PO_NUMBER = $scope.APViewSearch.PO_NUMBER;
        PaymentModelObj.PROJECT_MASTER_ID = $scope.TAB_FLAG == 1 ? $scope.APViewSearch.REQUEST_DETAILS.PROJECT_MASTER_ID : $scope.APViewSearch.ALL_REQUEST_DETAILS.PROJECT_MASTER_ID;
    ...
    $scope.$on('ngRepeatFinishedActionRender', function (ngRepeatFinishedEvent) {
        // if ($scope.APViewSearch.PO_DATE != undefined && $scope.APViewSearch.PO_DATE != "" && $scope.APViewSearch.PO_DATE != null && $scope.APViewSearch.PROJECT_MASTER_ID != undefined && $scope.APViewSearch.PROJECT_MASTER_ID != null && $scope.APViewSearch.PROJECT_MASTER_ID != "") {
        $scope.GET_P2P_BUDGET_BY_ACCOUNTS(1, $scope.APViewSearch.REQUEST_DETAILS, $scope.APViewSearch.REQUEST_DETAILS)

        //}
    });
    $scope.$on('ngRepeatFinishedProcessRender', function (ngRepeatFinishedEvent) {
        //if ($scope.APViewSearch.PO_DATE != undefined && $scope.APViewSearch.PO_DATE != "" && $scope.APViewSearch.PO_DATE != null && $scope.APViewSearch.PROJECT_MASTER_ID != undefined && $scope.APViewSearch.PROJECT_MASTER_ID != null && $scope.APViewSearch.PROJECT_MASTER_ID != "") {
        $scope.GET_P2P_BUDGET_BY_ACCOUNTS(2, $scope.APViewSearch.PROCESS_REQUEST_DETAILS, $scope.APViewSearch.PROCESS_REQUEST_DETAILS)
        if ($scope.INVOICE_PROCESS_SPLIT_LIST.length > 0) {
            $scope.Fn_SPLIT_PROCESS_ALL_OUT_INVOICE($scope.APViewSearch.PROCESS_REQUEST_DETAILS);
        }
        //}
    });
    $scope.$on('ngRepeatFinishedPoAllRender', function (ngRepeatFinishedEvent) {
        $scope.GET_P2P_BUDGET_BY_ACCOUNTS(3, $scope.APViewSearch.ALL_REQUEST_DETAILS, $scope.APViewSearch.ALL_REQUEST_DETAILS)
        if ($scope.INVOICE_PROCESS_SPLIT_LIST.length > 0) {
            $scope.Fn_SPLIT_PROCESS_ALL_OUT_INVOICE($scope.APViewSearch.ALL_REQUEST_DETAILS);
        }
    });
    $scope.$on('ngRepeatFinishedOutRender', function (ngRepeatFinishedEvent) {
        $scope.GET_P2P_BUDGET_BY_ACCOUNTS(4, $scope.APViewSearch.ALL_REQUEST_DETAILS, $scope.APViewSearch.ALL_REQUEST_DETAILS)
        if ($scope.INVOICE_PROCESS_SPLIT_LIST.length > 0) {
            $scope.Fn_SPLIT_PROCESS_ALL_OUT_INVOICE($scope.APViewSearch.OUT_REQUEST_DETAILS);
        }
    });
    ...
    $scope.Inittotalreq = function (parm_line, parm_m, index) {
        ...
        if ($scope.TAB_FLAG == 1) {
            $scope.ACCOUNT_ITEMDETAIL = itemdetail.filter(function (_m) { return new Date($scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE).getMonth() == new Date(_m.INVOICE_DATE).getMonth() && new Date($scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE).getFullYear() == new Date(_m.INVOICE_DATE).getFullYear() })
        }
        else if ($scope.TAB_FLAG == 3) {
            $scope.ACCOUNT_ITEMDETAIL = itemdetail.filter(function (_m) { return new Date($scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE).getMonth() == new Date(_m.INVOICE_DATE).getMonth() && new Date($scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE).getFullYear() == new Date(_m.INVOICE_DATE).getFullYear() })
        }
    }
    $scope.Inittotalproccereq = function (parm_line, parm_m, index) {
        ...
        if ($scope.TAB_FLAG == 2) {
            $scope.ACCOUNT_PROCESS_ALL_ITEMDETAIL = itemdetail.filter(function (_m) { return new Date($scope.APViewSearch.PROCESS_REQUEST_DETAILS.BUDGET_DATE).getMonth() == new Date(_m.INVOICE_DATE).getMonth() && new Date($scope.APViewSearch.REQUEST_DETAILS.BUDGET_DATE).getFullYear() == new Date(_m.INVOICE_DATE).getFullYear() })
        }
        else if ($scope.TAB_FLAG == 3) {
            $scope.ACCOUNT_PROCESS_ALL_ITEMDETAIL = itemdetail.filter(function (_m) { return new Date($scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE).getMonth() == new Date(_m.INVOICE_DATE).getMonth() && new Date($scope.APViewSearch.ALL_REQUEST_DETAILS.BUDGET_DATE).getFullYear() == new Date(_m.INVOICE_DATE).getFullYear() })
        }
    }
    ...
    $scope.VALIDATION_CLICK_FY = function (APPROVE_REJECT_FLAG) {
        ...
        if ($scope.TAB_FLAG == 1) {
            ALL_AND_ACTION = $scope.APViewSearch.REQUEST_DETAILS;
            ITEM_LIST = $scope.ACTION_ITEM_LIST;
            USE_INVOICE_GL = $scope.APViewSearch.REQUEST_DETAILS.USE_INVOICE_GL;
        }
        else if ($scope.TAB_FLAG == 3) {
            ALL_AND_ACTION = $scope.APViewSearch.ALL_REQUEST_DETAILS;
            ITEM_LIST = $scope.ALL_ITEM_LIST;
            USE_INVOICE_GL = $scope.APViewSearch.ALL_REQUEST_DETAILS.USE_INVOICE_GL;
        };
        ...
        if (IS_VALID) {
            $scope.APViewSearch.APPROVE_REJECT_FLAG = APPROVE_REJECT_FLAG;
            if ($scope.TAB_FLAG == 1) {
                $scope.SELECTED_INVOICE_DETAIL = $scope.APViewSearch.REQUEST_DETAILS;
                $scope.CONDITION_P2P_BUDGET_MASTER_LIST = $scope.P2P_BUDGET_MASTER_LIST;
                $scope.GET_PO_SPLIT_DATES_BY_INVOICE_ID($scope.APViewSearch.REQUEST_DETAILS);
                ...
                $scope.GET_PO_SPLIT_DATES_BY_INVOICE_ID($scope.APViewSearch.ALL_REQUEST_DETAILS);
            ...
            if ($scope.OTP_VALID_CHECK == '1' && $scope.NXT_APPROVAL_HEADER_STATUS_ID == '') {
                $scope.APViewSearch.digit1 = "";
                $scope.APViewSearch.digit2 = "";
                $scope.APViewSearch.digit3 = "";
                $scope.APViewSearch.digit4 = "";
                $scope.APViewSearch.digit5 = "";
                $scope.APViewSearch.digit6 = "";
    ...
    $scope.VALIDATION_FORCE_CLICK_FY = function (APPROVE_REJECT_FLAG, ACTIVE_TAB_APPROVAL_PROCESS) {
        $scope.APViewSearch.APPROVE_REJECT_FROCE_FLAG = APPROVE_REJECT_FLAG;
        $scope.ACTIVE_TAB_APPROVAL_PROCESS = ACTIVE_TAB_APPROVAL_PROCESS;
        var IS_VALID = true
        var Linecount = 0, AccountMatchCount = 0, EDIT_MATCHING = 0;
        if ($scope.APViewSearch.APPROVAL_TYPE_ID == 4 && APPROVE_REJECT_FLAG == 37) {
            ...
            else if ($scope.TAB_FLAG == 1 && $scope.ACTION_ITEM_LIST.filter(function (x) { return x.ACCOUNT_MATCH }).length != $scope.ACTION_ITEM_LIST.length && $scope.APViewSearch.REQUEST_DETAILS.USE_INVOICE_GL != true) {
                IS_VALID = false;
                AccountMatchCount = 1;
                $scope.$parent.ShowAlert('Error', 'The Invoice line GL Account is not matching the purchase order line GL Account', 3000);
            }
            else if ($scope.TAB_FLAG == 3 && $scope.ALL_ITEM_LIST.filter(function (x) { return x.ACCOUNT_MATCH }).length != $scope.ALL_ITEM_LIST.length && $scope.APViewSearch.ALL_REQUEST_DETAILS.USE_INVOICE_GL != true) {
                IS_VALID = false;
                AccountMatchCount = 1;
                $scope.$parent.ShowAlert('Error', 'The Invoice line GL Account is not matching the purchase order line GL Account', 3000);
            };
    ...
    $scope.VALIDATE_APPROVAL_OTP = function () {
        ...
        var ModelObj = new Object();
        ModelObj.APPROVAL_HEADER_ID = $scope.APViewSearch.APPROVAL_HEADER_ID;
        ModelObj.USER_ID = parseInt($cookies.get("USERID"));
        var digist = $scope.APViewSearch.digit1 + $scope.APViewSearch.digit2 + $scope.APViewSearch.digit3 + $scope.APViewSearch.digit4 + $scope.APViewSearch.digit5 + $scope.APViewSearch.digit6
        ModelObj.OTP = digist;
    ...
    $scope.PROJECT_POP_FY = function () {
        ...
        if ($scope.TAB_FLAG == 1) {
            $scope.GET_PROJECT_MASTER($scope.APViewSearch.REQUEST_DETAILS);
        }
        else if ($scope.TAB_FLAG == 3) {
            $scope.GET_PROJECT_MASTER($scope.APViewSearch.ALL_REQUEST_DETAILS);
        };
    ...
    $scope.Fn_CANCEL_SPLIT = function () {
        ...
        $scope.APViewSearch.SPLIT_TYPE = null;
    ...
    $scope.Fn_SPLIT_INVOICE = function (LINE, FLAG) {
        ...
        LINE = {};
        if ($scope.TAB_FLAG == 1) {
            LINE = $scope.APViewSearch.REQUEST_DETAILS;
        }
        else if ($scope.TAB_FLAG == 3) {
            LINE = $scope.APViewSearch.ALL_REQUEST_DETAILS;
        }
        if (FLAG == 1) {
            $scope.APViewSearch.SPLIT_TYPE = 1;
        }
        LINE.SPLIT_TYPE = $scope.APViewSearch.SPLIT_TYPE;
    ...
    $scope.Fn_SPLIT_PROCESS_ALL_OUT_INVOICE = function (LINE, FLAG) {
        ...
        if ($scope.TAB_FLAG == 2) {
            LINE = $scope.APViewSearch.PROCESS_REQUEST_DETAILS;
            $scope.APViewSearch.PROCESS_REQUEST_DETAILS.SPLIT_TYPE = _splittype;
        }
        else if ($scope.TAB_FLAG == 3) {
            LINE = $scope.APViewSearch.ALL_REQUEST_DETAILS;
            $scope.APViewSearch.ALL_REQUEST_DETAILS.SPLIT_TYPE = _splittype;
        }
        LINE.START_DATE = new Date(DATE);
        LINE.END_DATE = new Date(new Date(new Date(DATE).setMonth(11)).setDate(31));
    ...
    $scope.EDIT_REQ_PO_APPROVER_BTN = function (FLAG) {
        ...
        $('#EDIT_REQ').modal('show');
        if (FLAG == 1) {
            $scope.Edit_REQ_childScope.PAGE_LOAD_EDIT_REQ($scope.APViewSearch.REQUEST_DETAILS, 1, 'IS_APPROVER', $scope.SELECTED_REQ);
        }
        else if (FLAG == 3) {
            $scope.Edit_REQ_childScope.PAGE_LOAD_EDIT_REQ($scope.APViewSearch.ALL_REQUEST_DETAILS, 1, 'IS_APPROVER', $scope.SELECTED_REQ_ALL);
        }
    ...
    $scope.REMOVE_OTHER_FILTE = function (index, LINE) {
        $scope.$parent.DELETE_UPLOAD_ALL($scope.OTHER_ATTACHMENT, LINE, index, 2);
    }
    $scope.REMOVE_INVOICE_IN_FILTE = function (index, LINE) {
        $scope.$parent.DELETE_UPLOAD_ALL($scope.APViewSearch.REQUEST_DETAILS, LINE, index, 2);
    }
    $scope.REMOVE_INVOICE_IN_FILTE_ALL = function (index, LINE) {
        $scope.$parent.DELETE_UPLOAD_ALL($scope.APViewSearch.ALL_REQUEST_DETAILS, LINE, index, 2);
    }
    ...
    $scope.SET_INVOICE_APPROVAL = function () {
        ...
        if ($scope.TAB_FLAG == 1) {
            ALL_AND_ACTION = $scope.APViewSearch.REQUEST_DETAILS;
            ITEM_LIST = $scope.ACTION_ITEM_LIST;
            _remainingleft = parseFloat(parseFloat($scope.APViewSearch.REQUEST_DETAILS.CUSTOM_TOTAL_AMOUNT).toFixed(2)) != parseFloat(parseFloat(($scope.APViewSearch.REQUEST_DETAILS.MATCHED_PO_AMOUNT)).toFixed(2));
        }
        else if ($scope.TAB_FLAG == 3) {
            ALL_AND_ACTION = $scope.APViewSearch.ALL_REQUEST_DETAILS;
            ITEM_LIST = $scope.ALL_ITEM_LIST;
            ModelObj.COMMENTS = $scope.APViewSearch.COMMENTS;
            _remainingleft = parseFloat(parseFloat($scope.APViewSearch.ALL_REQUEST_DETAILS.CUSTOM_TOTAL_AMOUNT).toFixed(2)) != parseFloat(parseFloat(($scope.APViewSearch.ALL_REQUEST_DETAILS.MATCHED_PO_AMOUNT)).toFixed(2));
        };
        ...
        if ($scope.MAPPED_LIST.length == 0) { //--1 no po, 2 way match 3   3 way match
            ...
            if ($scope.TAB_FLAG == 1) {
                ModelObj.MATCH_TYPE = 1;
                if (parseFloat(parseFloat($scope.APViewSearch.REQUEST_DETAILS.CUSTOM_TOTAL_AMOUNT).toFixed(2)) <= parseFloat(parseFloat(toleranceamount * ($scope.APViewSearch.REQUEST_DETAILS.MATCHED_PO_AMOUNT)).toFixed(2))) {
                    ModelObj.MATCH_TYPE = 2;
                };
            }
            else if ($scope.TAB_FLAG == 3) {
                ModelObj.MATCH_TYPE = 1;
                if (parseFloat(parseFloat($scope.APViewSearch.ALL_REQUEST_DETAILS.CUSTOM_TOTAL_AMOUNT).toFixed(2)) <= parseFloat(parseFloat(toleranceamount * ($scope.APViewSearch.ALL_REQUEST_DETAILS.MATCHED_PO_AMOUNT)).toFixed(2))) {
                    ModelObj.MATCH_TYPE = 2;
                };
            }
        }
            ...
            if ($scope.TAB_FLAG == 1) {
                ModelObj.MATCH_TYPE = 1;
                if (parseFloat(parseFloat($scope.APViewSearch.REQUEST_DETAILS.CUSTOM_TOTAL_AMOUNT).toFixed(2)) == parseFloat(parseFloat(($scope.APViewSearch.REQUEST_DETAILS.MATCHED_PO_AMOUNT)).toFixed(2))) {
                    ModelObj.MATCH_TYPE = 2;
                };
            }
            else if ($scope.TAB_FLAG == 3) {
                ModelObj.MATCH_TYPE = 1;
                if (parseFloat(parseFloat($scope.APViewSearch.ALL_REQUEST_DETAILS.CUSTOM_TOTAL_AMOUNT).toFixed(2)) == parseFloat(parseFloat(($scope.APViewSearch.ALL_REQUEST_DETAILS.MATCHED_PO_AMOUNT)).toFixed(2))) {
                    ModelObj.MATCH_TYPE = 2;
                };
            }
        }
        ModelObj.USER_ID = parseInt($cookies.get("USERID"));
        ModelObj.COMMENTS = $scope.APViewSearch.COMMENTS;
        ModelObj.SPLIT_TYPE = $scope.APViewSearch.SPLIT_TYPE;
    ...
    $scope.INS_UPD_INVOICES = function (INVOICE_SYNC_LINE) {
        if (confirm('Are you sure you want to sync GL Account?')) {
            var ptopobj = new Object();
            ptopobj.INVOICE_HEADER_ID = $scope.APViewSearch.REQUEST_DETAILS.INVOICE_HEADER_ID;
            ptopobj.INVOICE_NUMBER = $scope.APViewSearch.REQUEST_DETAILS.INVOICE_NUMBER;
            ptopobj.REFERENCE = $scope.APViewSearch.REQUEST_DETAILS.REFERENCE;
            ptopobj.INVOICE_TYPE_ID = $scope.APViewSearch.REQUEST_DETAILS.INVOICE_TYPE_ID;
            ptopobj.CONTACT_ID = $scope.APViewSearch.REQUEST_DETAILS.CONTACT_ID;
            ptopobj.INVOICE_DATE = $scope.APViewSearch.REQUEST_DETAILS.INVOICE_DATE;
            ptopobj.DUE_DATE = $scope.APViewSearch.REQUEST_DETAILS.DUE_DATE;
            ptopobj.TAX_TYPE = $scope.APViewSearch.REQUEST_DETAILS.TAX_TYPE;
            ptopobj.NET_AMOUNT = parseFloat($scope.APViewSearch.REQUEST_DETAILS.NET_AMOUNT).toFixed(5); //NET;
            ptopobj.TAX_AMOUNT = parseFloat($scope.APViewSearch.REQUEST_DETAILS.TAX_AMOUNT).toFixed(5);//TAX;
            ptopobj.TOTAL_AMOUNT = parseFloat($scope.APViewSearch.REQUEST_DETAILS.TOTAL_AMOUNT).toFixed(5);//NET+TAX;
            ptopobj.TOTAL_DISCOUNT = parseFloat($scope.APViewSearch.REQUEST_DETAILS.TOTAL_DISCOUNT).toFixed(5);//NET+TAX;
            ptopobj.INVOICE_CURRENCY_ID = $scope.APViewSearch.REQUEST_DETAILS.CURRENCY_ID;
            ptopobj.BASE_CURRENCY_ID = $scope.APViewSearch.REQUEST_DETAILS.BASE_CURRENCY_ID;
            if ($scope.APViewSearch.REQUEST_DETAILS.CURRENCY_ID != $scope.APViewSearch.REQUEST_DETAILS.BASE_CURRENCY_ID) { ptopobj.BASE_TO_INVOICE_CONVERSION_RATE = $scope.APViewSearch.REQUEST_DETAILS.BASE_TO_INVOICE_CONVERSION_RATE; }
            else { ptopobj.BASE_TO_INVOICE_CONVERSION_RATE = 1; }
            ptopobj.XERO_BRANDING_THEME_ID = $scope.APViewSearch.REQUEST_DETAILS.XERO_BRANDING_THEME_ID == undefined ? null : $scope.APViewSearch.REQUEST_DETAILS.XERO_BRANDING_THEME_ID;
            ptopobj.XERO_INVOICE_ID = $scope.APViewSearch.REQUEST_DETAILS.XERO_INVOICE_ID;
            ptopobj.IS_DRAFT = 0;
            ptopobj.UPLOAD_IDS = "";
            angular.forEach($scope.APViewSearch.REQUEST_DETAILS.UploadedFiles, function (x) {
                if (ptopobj.UPLOAD_IDS == "") { ptopobj.UPLOAD_IDS = x.ID; }
                else { ptopobj.UPLOAD_IDS = ptopobj.UPLOAD_IDS + "," + x.ID; }
            });
            ptopobj.ENTITY_ID = parseInt($cookies.get("ENTITY_ID"));
            ptopobj.BRANCH_ID = $scope.APViewSearch.REQUEST_DETAILS.BRANCH_ID;
            ptopobj.USER_ID = parseInt($cookies.get("USERID"));
            ptopobj.INTEGRATION_SYSTEM_ID = null;
            ptopobj.PROJECT_MASTER_ID = $scope.APViewSearch.REQUEST_DETAILS.PROJECT_MASTER_ID;
    ...
    $scope.INS_UPD_PROJECT_IN_INVOICE = function () {
        ...
        if ($scope.project_form.$valid) {
            if (confirm('Are you sure you want to update ?')) {
                var ModelObj = new Object();
                if ($scope.TAB_FLAG == 1) {
                    ModelObj.INVOICE_HEADER_ID = $scope.APViewSearch.REQUEST_DETAILS.INVOICE_HEADER_ID;
                }
                else if ($scope.TAB_FLAG == 3) {
                    ModelObj.INVOICE_HEADER_ID = $scope.APViewSearch.ALL_REQUEST_DETAILS.INVOICE_HEADER_ID;
                }
                ModelObj.PROJECT_MASTER_ID = $scope.APViewSearch.PROJECT_MASTER_ID;
                PrcCommMethods.P2P_API(ModelObj, 'INS_UPD_PROJECT_IN_INVOICE', 'PO').then(function (data) {
                    ...
                    if (data.data != undefined && data.data == 1) {
                        ...
                        if ($scope.TAB_FLAG == 1) {
                            $scope.GET_INVOICE_BY_ID($scope.APViewSearch.REQUEST_DETAILS, 1);
                            $scope.GET_INVOICE_PO_MAPPING($scope.APViewSearch.REQUEST_DETAILS, 1);
                        }
                        if ($scope.TAB_FLAG == 3) {
                            $scope.GET_INVOICE_BY_ID($scope.APViewSearch.ALL_REQUEST_DETAILS, 3);

                            $scope.GET_INVOICE_PO_MAPPING($scope.APViewSearch.ALL_REQUEST_DETAILS, 3);
                        }
    ...
    $scope.UPD_USE_INVOICE_GL_FLAG = function () {
        ...
        if ($scope.TAB_FLAG == 1) {
            INVOICE_HEADER_ID = $scope.APViewSearch.REQUEST_DETAILS.INVOICE_HEADER_ID;
            USE_INVOICE_GL = $scope.APViewSearch.REQUEST_DETAILS.USE_INVOICE_GL ? 1 : 0;
        }
        else {
            INVOICE_HEADER_ID = $scope.APViewSearch.ALL_REQUEST_DETAILS.INVOICE_HEADER_ID;
            USE_INVOICE_GL = $scope.APViewSearch.ALL_REQUEST_DETAILS.USE_INVOICE_GL ? 1 : 0;
        }
        ModelObj = new Object();
    ...
    $scope.UPD_P2P_APPROVED_INVOICES_FOR_XERO_OUTBUND_INTEGRATION = function () {
        ModelObj = new Object();
        ModelObj.INVOICE_HEADER_ID = $scope.APViewSearch.OUT_REQUEST_DETAILS.INVOICE_HEADER_ID;
    ...
    $scope.AP_PROCESSED_CLICK = function (PROCESS_REQUEST) {
        $scope.APViewSearch.UploadedFiles = [];
        $scope.BUDGET_TEXT_VALIDATION = "Fetching Budget...";
    ...
    $scope.Fn_EDIT_INVOICE = function (FLAG) {
        if (FLAG == 1) {
            $scope.APViewSearch.REQUEST_DETAILS.TAB_FLAG = 1;
            $scope.APViewSearch.REQUEST_DETAILS.SELECTED_REQ = $scope.SELECTED_REQ;
            $scope.Invoice_childScope.PAGE_LOAD_INVOICE($scope.APViewSearch.REQUEST_DETAILS, FLAG);
        }
        else if (FLAG == 3) {
            $scope.APViewSearch.ALL_REQUEST_DETAILS.TAB_FLAG = 3;
            $scope.APViewSearch.ALL_REQUEST_DETAILS.SELECTED_REQ_ALL = $scope.SELECTED_REQ_ALL;
            $scope.Invoice_childScope.PAGE_LOAD_INVOICE($scope.APViewSearch.ALL_REQUEST_DETAILS, FLAG);
        }
        $scope.TAB_FLAG = 9;
    }
