app.controller('PNLDshBtUploadController', function ($scope, $http, $interval, CommService, $cookies, $filter, PrcCommMethods, $location, readFileData, $localStorage) {
    ...
    $scope.PNL_Upload_Search = {
        ...
        DISPLAY_TEXT_BRANCH_1: '--Select--',
        DISPLAY_TEXT_BRANCH_2: '--Select--',
    }
    ...
    function set_week_picker_filter(date, FLAG) {
        ...
        var EndDD = $scope.end_date_filter.getDate();
        var Endmm = $scope.end_date_filter.getMonth() + 1;
        var Endyyyy = $scope.end_date_filter.getFullYear();
    ...
    $scope.DATE_WEEK_PICKER_FILTER = function (date) {
        ...
        $('.week-filter-prev').on('click', function () {
            var prev = new Date($scope.start_date_filter.getTime());
            prev.setDate(prev.getDate() - 6);
            set_week_picker_filter(prev, 1);
        });
        $('.week-filter-next').on('click', function () {
            var next = new Date($scope.end_date_filter.getTime());
            next.setDate(next.getDate() + 1);
            set_week_picker_filter(next, 1);
        });
    ...
    $scope.DELETE_PNL_NOMINAL_AMOUNT_BUDGET_HEADER = function (LINE) {
        if (confirm('Are you sure you want to delete?')) {
            var POSModelObj = new Object();
            POSModelObj.HEADER_ID = LINE.ID;
            PrcCommMethods.DASHBOARD_MODULES_API(POSModelObj, 'DELETE_PNL_NOMINAL_AMOUNT_BUDGET_HEADER').then(function (data) {
                if (data.data == 1) {
    ...
    $scope.RESET_FILTER = function () {
        ...
        $scope.PNL_Upload_Search.DISPLAY_TEXT_BRANCH_1 = "--Select--";

        $scope.PNL_Upload_Search.PNL_COGS = "";
        $scope.PNL_Upload_Search.PNL_WAGE = "";
        $scope.PNL_Upload_Search.PNL_COST = "";
        var _day = $scope.$parent.addDays(new Date($scope.TODAY_DATE), -6);
        $scope.DATE_WEEK_PICKER_FILTER(new Date(_day));
    }
    $scope.RESET_INS = function () {
        ...
        $scope.PNL_Upload_Search.BRANCH_ID = null;
        $scope.PNL_Upload_Search.DISPLAY_TEXT_BRANCH_2 = "--Select--";
        $scope.RESET_FILE();
    ...
    $scope.INS_PNL_NOMINAL_AMOUNT_BUDGET_HEADER = function () {
        ...
        if ($scope.PNLForm.$valid && $scope.INV_RECO_UPLOAD_DETAILS_LIST.length > 0) {
            ...
            CustmObj.ENTITY_ID = $scope.PNL_Upload_Search.ENTITY_ID;
    ...
    $scope.RESET_FILE = function () {
        angular.element("input[id='PNL_recorduploadExcel1']").val(null);
        $scope.INV_RECO_UPLOAD_DETAILS_LIST = [];
        $scope.PNL_Upload_Search.UploadedFiles = [];
    }
    ...
    $scope.Fn_VIEW_ONLOAD_DATA = function (APPLY_METHOD, FLAG) {
        if ($scope.$parent.CURRENCY_LIST.length > 0 && (FLAG != 1 || $scope.Input_Model.CURRENCY_ID == undefined)) {
            ...
            //};
            $scope.LAND_BRANCH_CHANGE($scope.SITE_LIST[0], 1);
            $scope.LAND_BRANCH_CHANGE($scope.SITE_LIST[0], 2);
        };
    };

    $scope.LAND_BRANCH_CHANGE = function (site, FLAG) {
        var _entity_site = site.Key.split(',');
        if (FLAG == 1) {
            $scope.PNL_Upload_Search.DISPLAY_TEXT_BRANCH_1 = site.Value;
            $scope.PNL_Upload_Search.FILTER_BRANCH_ID = _entity_site[1];
            $scope.PNL_Upload_Search.ENTITY_ID = _entity_site[0];
        }
        else if (FLAG == 2) {

            $scope.PNL_Upload_Search.ENTITY_ID = _entity_site[0];
            $scope.PNL_Upload_Search.BRANCH_ID = _entity_site[1];
            $scope.PNL_Upload_Search.DISPLAY_TEXT_BRANCH_2 = site.Value;
        }
    }
    ...
    $scope.Fn_CLOSE_VIEW_REPORT = function () {
