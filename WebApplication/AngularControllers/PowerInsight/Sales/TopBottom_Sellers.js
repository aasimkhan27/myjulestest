app.controller('PI_TopBottom_SellersController', function ($scope, $http, CommService, $cookies, $filter, PrcCommMethods, $location, $localStorage) {
    ...
    $scope.DATE_QUARTER_VIEW_LOAD = function (eDate, FLAG) {
        var Quarters = $scope.$parent.getFiscalQuarterDates(new Date(eDate).getFullYear(), new Date(eDate)).split(',');
        $scope.Input_Model.START_DATE = new Date(Quarters[0]);
        $scope.Input_Model.END_DATE = new Date(Quarters[1]);
        $scope.Input_Model.WEEK_NO = Quarters[2];
        ...
        $scope.Input_Model.QUARTER_START_DATE = angular.copy(new Date(Quarters[0]));
        $scope.Input_Model.QUARTER_END_DATE = angular.copy(new Date(Quarters[1]));
        if (FLAG == 3) {
                    ...
                    date_input.datepicker(options).on("hide", function (e) {
                        if (e != undefined) {
                            if ($scope.Input_Model.PERIOD_TYPE_ID == 3) {
                                $scope.DATE_QUARTER_VIEW_LOAD(e.date);
                                $scope.Fn_CHANGE_FILTER();
                            }
                        }
                    });
    ...
    $scope.GetChartData = function () {
        ...
        var _pi_cat_obj = new Object();
        _pi_cat_obj.START_DATE = new Date($scope.Input_Model.START_DATE).toDateString();
        _pi_cat_obj.END_DATE = new Date($scope.Input_Model.END_DATE).toDateString();
        _pi_cat_obj.ENTITY_BRANCH_LIST = $scope.Input_Model.ENTITY_BRANCH_LIST;

        PrcCommMethods.DASHBOARD_MODULES_API(_pi_cat_obj, 'GET_EPOS_CATEGORY', 'POWERINSIGHTAPI').then(function (data) {
            $scope.GET_EPOS_CATEGORY = data.data.Table;

            angular.forEach(data.data.Table, function (_category) {
                _category.CATERGORY_TOP_DTLS = [];
                _category.CATERGORY_BOTTOM_DTLS = [];

                $scope.$parent.$parent.overlay_loading_powerInsight = 'block';
                $scope.$parent.GET_CONVERSION_RATES_BY_BRANCH_FOR_POWER_INSIGHTS($scope.Input_Model.START_DATE, $scope.Input_Model.END_DATE, $scope.Input_Model.CURRENCY_ID, $scope.Input_Model.ENTITY_BRANCH_LIST, $scope.Input_Model.DISPLAY_TEXT_CURRENCY);
                $scope.Display_Model = {};
                angular.forEach($scope.THIS_APP_PAGE_COMPONENTS_LIST, function (component, index) {
                    // if (component.COMPONENT_ID == 119) {
                    ...
                    _pi_input_model_obj.CATEGORY_NAME = _category.CATEGORY_NAME;
                    ...
                    _pi_input_model_obj.SORT_ORDER = component.COMPONENT_TYPE_ID == 119 ? -1 : 1;//asc =1 bott0m//desc -1 top;
                     //119  ttop hain sort  
                     //120 top
                    ...
                    PrcCommMethods.DASHBOARD_MODULES_API(_pi_input_model_obj, 'GETDATAFORCHART', 'POWERINSIGHTAPI').then(function (data) {
                        ...
                        $scope.CardLineData = [];
                        if (data.data != null) {
                            $scope.SetDisplayObjects(data.data, _category);
    ...
    $scope.SetDisplayObjects = function (data, _category) {
        switch (data.COMPONENT_TYPE_ID) {
            case 119:
                $scope.Display_Model.TOP_TITLE = data.TITLE;
                _category.CATERGORY_TOP_DTLS = data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST;
                _category.TOP = data;
                _category.AMOUNT = data.MENUPROFITABILITY_OBJECT_LIST[0].AMOUNT;
                if (_category.CATERGORY_TOP_DTLS.length < 10) {
                    _category.GetData = false;
                }
                else {
                    _category.PAGE_NO = 1;
                    _category.GetData = true;
                }
                break;
            case 120:
                $scope.Display_Model.BOTTOM_TITLE = data.TITLE;
                _category.CATERGORY_BOTTOM_DTLS = data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST;
                _category.BOTTOM = data;
                _category.AMOUNT = data.MENUPROFITABILITY_OBJECT_LIST[0].AMOUNT;
                if (_category.CATERGORY_BOTTOM_DTLS.length < 10) {
                    _category.BottomGetData = false;
                }
                else {
                    _category.BOTTOM_PAGE_NO = 1;
                    _category.BottomGetData = true;
                }
                break;
        }
    }

    $scope.Get_TOP_Bottom_Data = function (_category, component, TOP_BOTTOM_FLAG) {
        $scope.$parent.$parent.overlay_loading_powerInsight = 'block';
        ...
        _pi_input_model_obj.COMPONENT_ID = component.COMPONENT_ID;
        _pi_input_model_obj.COMPONENT_TYPE_ID = component.COMPONENT_ID;// for now i am passing component id- component.COMPONENT_TYPE_ID;
        _pi_input_model_obj.SP_NAME = component.SP_NAME;
        _pi_input_model_obj.SP_PARAMETERS = component.SP_PARAMETERS;
        ...
        _pi_input_model_obj.SORT_ORDER = component.COMPONENT_TYPE_ID == 119 ? -1 : 1;//asc =1 bottom //desc -1 top;
        if (TOP_BOTTOM_FLAG == 1) {
            _category.PAGE_NO == undefined ? _category.PAGE_NO = 1 : '';
            _pi_input_model_obj.PAGE_NO = _category.PAGE_NO + 1;
        }
        else if (TOP_BOTTOM_FLAG == 2) {
            _category.BOTTOM_PAGE_NO == undefined ? _category.BOTTOM_PAGE_NO = 1 : '';
            _pi_input_model_obj.PAGE_NO = _category.BOTTOM_PAGE_NO + 1;
        }
        ...
        PrcCommMethods.DASHBOARD_MODULES_API(_pi_input_model_obj, 'GETDATAFORCHART', 'POWERINSIGHTAPI').then(function (data) {
            if (data.data != null) {
                if (TOP_BOTTOM_FLAG == 1) {
                    if (data.data.MENUPROFITABILITY_OBJECT_LIST.length > 0 && data.data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST.length > 0) {
                        _category.CATERGORY_TOP_DTLS = _category.CATERGORY_TOP_DTLS.concat(data.data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST);
                        if (data.data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST.length < 10) {
                            _category.GetData = false;
                        }
                        else {
                            _category.PAGE_NO = parseInt(_category.PAGE_NO) + 1;
                            _category.GetData = true;
                        }
                    }
                    else {
                    ...
                    if (data.data.MENUPROFITABILITY_OBJECT_LIST.length > 0 && data.data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST.length > 0) {
                        _category.CATERGORY_BOTTOM_DTLS = _category.CATERGORY_BOTTOM_DTLS.concat(data.data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST);
                        if (data.data.MENUPROFITABILITY_OBJECT_LIST[0].MENUPROFITABILITY_OBJECT_LIST.length < 10) {
                            _category.BottomGetData = false;
                        }
                        else {
                            _category.BOTTOM_PAGE_NO = parseInt(_category.BOTTOM_PAGE_NO) + 1;
                            _category.BottomGetData = true;
                        }
                    }
                    else {
