app.controller('InvRec_Prepayment_JV_Controller', function ($scope, $http, $interval, CommService, $cookies, $filter, PrcCommMethods, $location, readFileData, $localStorage) {
    $(".tooltip").remove();
    ...
    $scope.INV_RECO_PREPAYMENT_JOURNALS_LIST = [];
    $scope.GET_INV_RECO_JOURNALS = function (HEADER_DTLS) {
        ...
        PrcCommMethods.CASHUP_API(CustmObj, 'GET_INV_RECO_JOURNALS').then(function (data) {
            if (data.data.Table.length > 0) {
                $scope.INV_RECO_PREPAYMENE_HEADER_DTLS = data.data.Table[0];
                $scope.INV_RECO_PREPAYMENT_JOURNALS_LIST = data.data.Table1;
    ...
    $scope.NEXT_STEP = function () {
        $location.path('InvRec_Finalize').search('HEADER_ID', $scope.HEADER_ID);
    }
