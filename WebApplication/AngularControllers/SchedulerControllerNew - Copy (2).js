app.controller('SchedulerMainController', function ($scope, $http, CommService, $cookies, $filter, PrcCommMethods, $location, $localStorage) {
    ...
    $scope.Fn_CHANGE_CLOCKINOUT = function () {
        if ($scope.SELECTED_CELL != undefined) {
            ...
            var ms = moment(END_DATE, "DD/MM/YYYY HH:mm:ss").diff(moment(START_DATE, "DD/MM/YYYY HH:mm:ss"));
            var diffSchDuration = moment.duration(ms);
            var ms = moment($scope.Section_Week_Search.EMPLOYEE_LOGOUT, "DD/MM/YYYY HH:mm:ss").diff(moment($scope.Section_Week_Search.EMPLOYEE_LOGIN, "DD/MM/YYYY HH:mm:ss"));
            var diffActDuration = moment.duration(ms);

            $scope.ScheduledHours = diffSchDuration._data.hours;
            let Scheduledminutes = diffSchDuration._data.minutes > 0 ? (diffSchDuration._data.minutes / 60).toFixed(2) : 0;
            $scope.ScheduledHours = parseFloat($scope.ScheduledHours) + parseFloat(Scheduledminutes);

            $scope.ActualHours = diffActDuration._data.hours;
            let Actualminutes = diffActDuration._data.minutes > 0 ? (diffActDuration._data.minutes / 60).toFixed(2) : 0;
            // ((((BREAK_LIST | filter: FILTERBYUNPAID) | total: 'DURATION')
            var Break = 0;
            if ($scope.BREAK_LIST.length > 0) {
                Break = $filter('total')(($scope.BREAK_LIST.filter(function (_parm_bl) { return _parm_bl.BREAK_TYPE_ID == 1 && _parm_bl.DELETE_FLAG != 1 })), 'DURATION')
            }
            $scope.ActualHours = parseFloat($scope.ActualHours) + parseFloat(Actualminutes) - ($scope.$parent.CheckSubModuleAccess(103) ? parseFloat(Break / 60) : 0);
            $scope.Section_Week_Search.ActualHours = $scope.ActualHours;
            //  $scope.$parent.CheckSubModuleAccess(103) ? 1 : 0
        }
    }
        ...
        if ($scope.APPROVE_FLAG == 1) {
            if ($scope.Section_Week_Search.EMPLOYEE_LOGIN == undefined || $scope.Section_Week_Search.EMPLOYEE_LOGIN == null || $scope.Section_Week_Search.EMPLOYEE_LOGIN == '') {
                ...
                if (alertBox == true) {
                    ...
                    $scope.ActualHours = diffActDuration._data.hours;
                    let Actualminutes = diffActDuration._data.minutes > 0 ? (diffActDuration._data.minutes / 60).toFixed(2) : 0;
                    $scope.ActualHours = parseFloat($scope.ActualHours) + parseFloat(Actualminutes) - ($scope.$parent.CheckSubModuleAccess(103) ? parseFloat($scope.Section_Week_Search.UN_PAID_BREAK / 60) : 0);

                    //$scope.ActualHours = parseFloat($scope.ActualHours) + parseFloat(Actualminutes) - ($scope.$parent.CheckSubModuleAccess(103) ? ($scope.Section_Week_Search.UN_PAID_BREAK) : 0);
                };
            }
        }
        else {
...
app.controller('New_AreaWeekViewController', function ($scope, $http, CommService, $cookies, $filter, PrcCommMethods, $location, $localStorage) {
    ...
    $scope.AREA_ROTA_GET_SECTION = function () {
        ...
        PrcCommMethods.HR_API(ModelObj, 'ROTA_GET_SECTION_OBJ').then(function (data) {
            console.log("API End  Time=>" + new Date())
            $scope.GET_WAGE_COST_FORECASTING();
    ...
    $scope.nginitShiftweek = function (DPT, Cell, Shift) {
        if ($scope.LEAVE_LIST != undefined && $scope.LEAVE_LIST.length > 0) {
    ...
    $scope.INS_UPD_BRANCH_BUDGETS = function () {
        ...
        if ($scope.RevenueForm.$valid && $scope.DAILY_BUDGET_LIST.length > 0) {
            ...
            angular.forEach($scope.DAILY_BUDGET_LIST, function (val) {
                ...
                ReadModelObj.FORCASTED_SALES =null;
                ReadModelObj.FORCASTED_COVERS = null;
