app.controller('CashBankingController', function ($scope, $http, CommService, $cookies, $filter, PrcCommMethods, $location) {
    ...
    //    CommonObj.USER_ID = $scope.CASH_BANKING_SEARCH.USER_ID;
    //    PrcCommMethods.CASHUP_APP_API(CommonObj, 'GET_USER_ROLES_BY_USER_ID').then(function (data) {
    //        if (data.data.Table.filter(_row => _row.ENTITY_ID == parseInt($cookies.get("ENTITY_ID"))).length > 0) {
    //            $scope.USER_ROLES_BY_USER_ID = data.data.Table.filter(_row => _row.ENTITY_ID == parseInt($cookies.get("ENTITY_ID")));
    //        }
    ...
    //    var CommonObj = new Object();
    //    CommonObj.MODULE_ID = $scope.CASH_BANKING_SEARCH.MODULE_ID;
    //    CommonObj.ENTITY_ID = $scope.CASH_BANKING_SEARCH.ENTITY_ID;
    ...
    //            $scope.SITES_LIST = $scope.USER_ROLES_BY_USER_ID;
    //            //$scope.SITES_LIST = $scope.$parent.$parent.GET_USER_ROLES_BY_USER_ID();

    //            var hasRequiredPrivilege = $scope.PRIVILAGE_LIST.some(function (priv) {
    //                return priv.PRIVILEGE_ID == "1171";
    //            });

    //            if (hasRequiredPrivilege) {
    //                $scope.FILTERED_SITES_LIST = $scope.SITES_LIST.filter(function (site) {
    //                    if (!site.USER_PRIVILEGE_IDS) return false;
    //                    var userPrivileges = site.USER_PRIVILEGE_IDS.replace(/^,/, '').split(',');
    //                    return userPrivileges.includes("1171");
    //                });
    ...
    //                    $scope.GET_CASHUP_TYPE($scope.FILTERED_SITES_LIST_FOR_BANKING);

    //                }
    //            } else {
    //                console.warn("User does not have required PRIVILEGE_ID 1171.");
    //            }
    //        }
    //    });
    //};
