app.controller('HRIndexController', function ($scope, $http, CommService, $cookies, $filter, PrcCommMethods, $location, $interval, $localStorage, $window) {
    ...
    $scope.EFFECTIVE_DATE_PICKER = function (MY_LOCK_DATE) {
        $(document).ready(function () {
            var date_start_inputs = document.getElementsByClassName("EFFECTIVE_DATE") //our date input has the name "date"
            if (date_start_inputs.length > 0) {
                for (var i = 0; i < date_start_inputs.length; i++) {
                    ...
                    const dateStart = new Date(MY_LOCK_DATE);
    ...
    $scope.GET_HRM_LOCK_DATE = function (_BRANCH_ID) {
        ...
        PrcCommMethods.HUMANRESOURCE_API(UserModelObj, 'GET_HRM_LOCK_DATE').then(function (data) {
            if (data.data.Table.length > 0) {
                ...
                $localStorage.HRM_LOCK_DATE_LIST = JSON.stringify(data.data.Table);
                if (_BRANCH_ID !== undefined) {
                    $scope.GET_HRM_LOCK_DATE(_BRANCH_ID);
                }
            }
        });
    }
    $scope.CHECK_LOCK_DATE_Fn = function (_BRANCH_ID) {
        if ($localStorage.HRM_LOCK_DATE_LIST == undefined) {
            $scope.GET_HRM_LOCK_DATE(_BRANCH_ID);
        }
        else {
            $scope.HRM_LOCK_DATE_LIST = JSON.parse($localStorage.HRM_LOCK_DATE_LIST);
            var LOCK_DATE = $scope.HRM_LOCK_DATE_LIST.filter(function (x) { return x.BRANCH_ID == _BRANCH_ID });
            return LOCK_DATE[0].LOCK_DATE
        };
    }
    ...
    $scope.POP_EFFECTIVE_TAB_Fn = function (_Flag) {
        $scope.POP_EFFECTIVE_TAB = _Flag;
    }
...
//document.getElementById('searchableDropdownButton').addEventListener('mouseenter', function () {
//    this.focus();
//});
