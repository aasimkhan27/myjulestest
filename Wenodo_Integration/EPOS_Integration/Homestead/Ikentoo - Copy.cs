...
namespace EPOS_Integration.LightSpeed
{
    ...
    public class Ikentoo
    {
        ...
        public void SaveDataToDB(DataTable dt, DataTable dt_IntegrationDetails)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (DataRow dr_session in dt_Session.Select("USE_IN_CASHUP=true").CopyToDataTable().Rows)
                {
                    ...
                    try
                    {
                        Root FinancialData = FetchFinancialDataByTime(Cashup_start_date, Cashup_end_date, dtIntegrationData);
                        if (FinancialData != null)
                        {
                            DataSet ds = Obj.GET_CASHUP_BY_ID();
                            INTEGRATION_STATUS = SubmitdataFromIkento(FinancialData, Convert.ToDecimal(ds.Tables[0].Rows[0]["ID"]), Obj.CashupModelObj.SESSION_ID, Convert.ToDecimal(ds.Tables[0].Rows[0]["INTEGRATION_SYSTEM_ID"]), Convert.ToDecimal(dr["ENTITY_ID"]));
        ...
        Root FetchFinancialDataByTime(string Start_date, string End_date, DataTable dtIntegrationData)
        {
            try
            {
                //  date = "2021-01-03";
                ...
                string URL = MainIkentooURL + "financials/" + Start_date + "/" + End_date + "?includeConsumers=true&include=staff,table,consumer,revenue_center,account_profile,payment_authorization,payments&pageSize=50000";
        ...
        int SubmitdataFromIkento(Root FinancialData, decimal CashupHeaderID, decimal SessionID, decimal INTEGRATION_SYSTEM_ID, decimal ENTITY_ID)
        {
            try
            {
                ...
                //   DataTable dt_EPOSDATA = ToDataTables(EPOS_DATA_LIST);
                //       DataTable dt_PAYMENT = ToDataTables(EPOS_PAYMENTS_LIST);
