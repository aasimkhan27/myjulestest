...
using EPOS_Integration.Integrations.GenericIntegrations;
using EPOS_Integration.Model;

namespace EPOS_Integration.Integrations.Iiko
{
    public class IkoIntegrator: Integrator, IIkoIntegrator
    {
        ...
        public IkoIntegrator(HttpClient httpClient, bool enableInformationLogging= false): base(httpClient, enableInformationLogging)
        {
            _stores = new JArray();
        ...
        public async Task Logout()
        {
            ...
            var builder = new UriBuilder {Host = _baseUrl, Path = path};
            var resp =  await Get<JObject>(builder.Uri, BearerToken);
            BearerToken = string.Empty;
        }
