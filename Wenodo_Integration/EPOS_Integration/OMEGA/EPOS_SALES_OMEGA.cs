...
namespace EPOS_Integration.OMEGA
{
    public class TransformOmegaData<T>
    {
        ...
        DataTable FillHeader(Root_Omega root_Omega, Cashup _ICashUp)
        {
            try
            {
                foreach (var OMEGA_HEADER in root_Omega.data.headers)
                {
                    ...
                    if (_ICashUp.CashupModelObj.EPOS_SALES_LINES.Copy().AsEnumerable().Where(p => p.Field<string>("CHECK_ID") == OMEGA_HEADER.HEADER_ID.ToString()).Count() > 0)
                    {
                        ...
                        DR_OMEGA_HEADER["NET"] = SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").Count() > 0 ? SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").CopyToDataTable().Compute("SUM(NET)", string.Empty) : 0;//NET_TOTAL;// root_Omega.data.details.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.TOTALNETAMOUNTWITHOUTTAX).Equals(null) ? (object)DBNull.Value : root_Omega.data.details.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.TOTALNETAMOUNTWITHOUTTAX);
                        DR_OMEGA_HEADER["TAX"] = SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").Count() > 0 ? SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").CopyToDataTable().Compute("SUM(TAX)", string.Empty) : 0;//SUM_OF_TAX;// HEADER_TAX;//root_Omega.data.details.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.TAXAMOUNT).Equals(null) ? (object)DBNull.Value : root_Omega.data.details.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.TAXAMOUNT);
                        DR_OMEGA_HEADER["GROSS"] = SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").Count() > 0 ? SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").CopyToDataTable().Compute("SUM(GROSS)", string.Empty) : 0;// NET_TOTAL + SUM_OF_TAX; //root_Omega.data.payments.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.NETAMOUNTWITHTAX).Equals(null) ? (object)DBNull.Value : root_Omega.data.payments.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.NETAMOUNTWITHTAX);
                        DR_OMEGA_HEADER["DISCOUNT"] = SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").Count() > 0 ? SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").CopyToDataTable().Compute("SUM(DISCOUNT)", string.Empty) : 0;// TOTAL_DISC_CPN;//root_Omega.data.discounts.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.DISCOUNTAMOUNT).Equals(null) ? (object)DBNull.Value : root_Omega.data.discounts.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID).Sum(p => p.DISCOUNTAMOUNT);
                        ...
                        DR_OMEGA_HEADER["VOID"] = SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").Count() > 0 ? SALES_LINE.Select("CHECK_ID='" + OMEGA_HEADER.HEADER_ID + "'").CopyToDataTable().Compute("SUM(VOID)", string.Empty) : 0;// root_Omega.data.details.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID && p.VOIDID != null).Sum(p => p.TOTALNETAMOUNTWITHOUTTAX).Equals(null) ? 0 : (root_Omega.data.details.Where(p => p.HEADER_ID == OMEGA_HEADER.HEADER_ID && p.VOIDID != null).Sum(p => p.TOTALNETAMOUNTWITHOUTTAX))*-1;
        ...
        DataTable FillLines(Root_Omega root_Omega, int Omega_Header_Id, Cashup _ICashUp)
        {
            try
            {
                ...
                foreach (var OMEGA_LINE in root_Omega.data.details.Where(p => p.HEADER_ID == Omega_Header_Id))
                {
                    ...
                    DR_OMEGA_DETAILS["PRODUCT_SKU"] = OMEGA_LINE.SALESITEMID;
                    ...
                    DR_OMEGA_DETAILS["VOID"] = Convert.ToDecimal(OMEGA_LINE.VOIDID) != 0 ? (Convert.ToDecimal(OMEGA_LINE.TOTALNETAMOUNTWITHOUTTAX) > 0 ? (OMEGA_LINE.TOTALNETAMOUNTWITHOUTTAX) * -1 : (OMEGA_LINE.TOTALNETAMOUNTWITHOUTTAX)):0;
                    ...
                    DR_OMEGA_DETAILS["STAFF_NAME"] = OMEGA_LINE.STAFFNAME == "" ? (object)DBNull.Value : OMEGA_LINE.STAFFNAME;
                    DR_OMEGA_DETAILS["VOID_ID"] = OMEGA_LINE.VOIDID == null ? (object)DBNull.Value : OMEGA_LINE.VOIDID;
                    DR_OMEGA_DETAILS["VOID_REASON"] = OMEGA_LINE.VOIDID == null ? (object)DBNull.Value : OMEGA_LINE.VOIDDESCRIPTION;
                    DR_OMEGA_DETAILS["DISCOUNT_ID"] = root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id).Count() > 0 ? string.Join(",", root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id).Select(p => p.DISCOUNTID).Distinct()) : (object)DBNull.Value;
                    DR_OMEGA_DETAILS["DISCOUNT_REASON"] = root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id).Count() > 0 ? string.Join(",", root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id).Select(p => p.DISCOUNTDESCRIPTION).Distinct()) : (object)DBNull.Value;
                    DR_OMEGA_DETAILS["DISCOUNT_RATE"] = LINE_DISCOUNT == 0 ? 0 : (LINE_DISCOUNT / (LINE_NET + LINE_DISCOUNT)) * 100; //TEMP_DIS_PER * 100;
        ...
        DataTable FillPayments(Root_Omega root_Omega, int Omega_Header_Id, Cashup _ICashUp)
        {
            try
            {
                ...
                { 
                    ...
                    DR_OMEGA_PAYMENT["TOTAL_AMOUNT_WITH_TIPS"] = Convert.ToDecimal(OMEGA_PAYMENT.NETAMOUNTWITHTAX) + Convert.ToDecimal(OMEGA_PAYMENT.TIP);
        ...
        DataTable FillDiscounts(Root_Omega root_Omega, int Omega_Header_Id, Cashup _ICashUp)
        {
            try
            {
                var DISTINCT_DISCOUNT_IDS = root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id).Select(p => p.DISCOUNTID).Distinct();

                foreach (var dist in DISTINCT_DISCOUNT_IDS)
                {
                    if (root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Count() > 0)
                    {
                        if (root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Sum(p => p.DISCOUNTAMOUNT) !=0)
                        {

                            DataRow DR_OMEGA_DISCOUNT = _ICashUp.CashupModelObj.EPOS_SALES_DISCOUNTS.NewRow();
                            DR_OMEGA_DISCOUNT["CHECK_ID"] = Omega_Header_Id;
                            DR_OMEGA_DISCOUNT["DISCOUNT_ID"] = dist;
                            DR_OMEGA_DISCOUNT["DISCOUNT_DESCRIPTION"] = root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Select(p => p.DISCOUNTDESCRIPTION).Count() > 0 ? root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Select(p => p.DISCOUNTDESCRIPTION).FirstOrDefault() : (object)DBNull.Value;
                            DR_OMEGA_DISCOUNT["DISCOUNT_AMOUNT"] = root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Count() > 0 ? root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Sum(p => p.DISCOUNTAMOUNT) : 0;
                            DR_OMEGA_DISCOUNT["STAFF_ID"] = root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Select(p => p.STAFFID).Count() > 0 ? root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Select(p => p.STAFFID).FirstOrDefault() : (object)DBNull.Value;
                            DR_OMEGA_DISCOUNT["STAFF_NAME"] = root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Select(p => p.STAFFNAME).Count() > 0 ? root_Omega.data.discounts.Where(p => p.HEADER_ID == Omega_Header_Id && p.DISCOUNTID == dist).Select(p => p.STAFFNAME).FirstOrDefault() : (object)DBNull.Value;
                ...
                //    DR_OMEGA_DISCOUNT["STAFF_NAME"] = OMEGA_DISCOUNT.STAFFNAME == ""? (object)DBNull.Value : OMEGA_DISCOUNT.STAFFNAME;
                //    _ICashUp.CashupModelObj.EPOS_SALES_DISCOUNTS.Rows.Add(DR_OMEGA_DISCOUNT);
                //}
