    ...
    public class Simphony
    {
                ...
                foreach (DataRow dr_Dist_BranchID in _filter_Dist_BranchID.Rows)
                {
                    ...
                    if (dt.Select("BRANCH_ID=" + Convert.ToDecimal(dr_Dist_BranchID["BRANCH_ID"])).Count() > 0)
                    {
                        foreach (DataRow dataRow in dt.Select("BRANCH_ID=" + Convert.ToDecimal(dr_Dist_BranchID["BRANCH_ID"])).CopyToDataTable().Rows)
                        {
                            ...
                            if (_refreshTokenTable.Rows.Count == 0)
                            {
            ...
            try
            {
                ...
                if (root != null && root.guestChecks.Count > 0)
                {
                    if (Convert.ToString(Int_DataTable.Rows[0]["URL_PARAMETERS"].ToString()).Split(_stringSeparators, StringSplitOptions.RemoveEmptyEntries).Length > 4)
                    {
                        ...
                        IList<SIMPHONY_TABLES.MICROS_SIMPHONY_GUEST_CHECKS_TYPE> _micros_Simphony_Guest_Checks_Type = root.guestChecks.Where(x => x.rvcNum == Convert.ToInt32(Convert.ToString(Int_DataTable.Rows[0]["URL_PARAMETERS"].ToString()).Split(_stringSeparators, StringSplitOptions.RemoveEmptyEntries)[4])).Select(p => new SIMPHONY_TABLES.MICROS_SIMPHONY_GUEST_CHECKS_TYPE
                        {
                        ...
                        IList<SIMPHONY_TABLES.MICROS_SIMPHONY_GUEST_CHECKS_DETAIL_LINE_TYPE> _micros_Simphony_Guest_Checks_Detail_Line_Types = root.guestChecks.Where(x => x.rvcNum == Convert.ToInt32(Convert.ToString(Int_DataTable.Rows[0]["URL_PARAMETERS"].ToString()).Split(_stringSeparators, StringSplitOptions.RemoveEmptyEntries)[4])).SelectMany(p => p.detailLines.Select(x => new SIMPHONY_TABLES.MICROS_SIMPHONY_GUEST_CHECKS_DETAIL_LINE_TYPE
                        {
                        ...
                        IList<SIMPHONY_TABLES.MICROS_SIMPHONY_GUEST_CHECKS_TAXES_TYPE> _micros_Simphony_Guest_checks_Taxes_Types = root.guestChecks.Where(x => x.rvcNum == Convert.ToInt32(Convert.ToString(Int_DataTable.Rows[0]["URL_PARAMETERS"].ToString()).Split(_stringSeparators, StringSplitOptions.RemoveEmptyEntries)[4])).SelectMany(p => p.taxes.Select(x => new SIMPHONY_TABLES.MICROS_SIMPHONY_GUEST_CHECKS_TAXES_TYPE
                        {
                            TAX_NUM = x.taxNum,
        ...
        public void GetRefreshToken_API(DataRow dataRow,DataTable dt_IntegrationDetails, bool IS_OUTBOUND = false)
        {
            ...
            try
            {
                ...
                {
                    {
                        if (Convert.ToInt32(dt_RecordsToUpdate.Rows[0]["ENTITY_ID"]) == 117 && Obj.CashupModelObj.INTEGRATION_SYSTEM_ID == 27)
                        {
                            foreach (DataRow dr in dt_IntegrationDetails.Select("ENTITY_ID=117 and IS_OUTBOUND = '" + IS_OUTBOUND + "' and INTEGRATION_SYSTEM_ID=27").CopyToDataTable().Rows)
                            {
