...
{
    public class QuadranetTransformer : BaseTransformer, IDataTransformer
    {
        public void Transform(DataTable integrationData, object data, decimal cashupMainId, Cashup cashup)
        {
            var obj = new EPOS_SALES_QUADRANET<DataSet>();
            obj.CashupModelObj = cashup.CashupModelObj;
            cashup.CashupModelObj = obj.Transform_Quadranet(data as DataSet, (int)IntegrationSource.QUADRANET, cashup, integrationData);
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                cashup.CashupModelObj.ERROR_MESSAGE = "Exception From Epos_Sales(QUADRANET) Common.--------" + ex.Message.ToString();
                throw;
            }
        }
    }
